    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-200/80 bg-white/90 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/90 shadow-sm transition-colors">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800">
                  Supabase Live
                </span>
              </div>
              <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                {isRealtime ? (
                  ...
                  <span className="flex items-center gap-1 text-amber-500">
                    <WifiOff className="h-3 w-3" /> ซิงค์อัตโนมัติ
                  </span>
                )}
              </div>
            </div>
          ...
          {/* Right Actions: Fuel pill & Theme Toggle */}
          <div className="flex items-center gap-2 sm:gap-3">
            {/* Live Fuel Price Badge */}
            <button
              onClick={onOpenFuelModal}
              title="ตั้งค่าน้ำมันและดูราคาสด"
              className="flex items-center gap-1.5 sm:gap-2 rounded-lg border border-slate-200 bg-slate-50/80 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800/80 dark:text-slate-200 dark:hover:bg-slate-800 transition shadow-sm"
            >
              <Fuel className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400" />
              <span className="hidden sm:inline text-slate-500 dark:text-slate-400">{brandName} {fuelName}:</span>
            ...
            <button
              ...
              className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-slate-200 transition"
            >
          ...
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 sm:gap-2 rounded-lg px-3.5 py-2 text-xs sm:text-sm font-semibold transition-all shrink-0 ${
                  isActive
                    ? 'bg-emerald-600 text-white shadow-md shadow-emerald-600/20 dark:bg-emerald-500 dark:text-slate-950'
                    : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'
                }`}
              >
                <Icon className={`h-4 w-4 ${isActive ? 'text-white dark:text-slate-950' : 'text-slate-500 dark:text-slate-400'}`} />
                {tab.label}
              </button>
            );
          })}
